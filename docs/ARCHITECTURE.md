...
| | `OLLAMA_VERIFY_SSL` | `true` | Проверка TLS-сертификата при скачивании моделей (отключить за корп. прокси) |
...
| | `LANGSMITH_VERIFY_SSL` | `true` | Проверка TLS-сертификата LangSmith (отключить за корп. прокси) |
